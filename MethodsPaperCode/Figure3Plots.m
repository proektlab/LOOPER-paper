...
% 
% clusterDistances = pdist2(tempData', app.SavedData.FinalStream);
% 
% MIN_RETURN_TIME = 10;
% 
% 
...
% originalDiff = [];
% for j = 1:length(trialData)
%     thisTrial = app.SavedData.TrialTimeSeries{j}';
...
%     
%     originalStream = (app.SavedData.TimeSeries' - tempData(:,i)') ./ currentSTD;
% 
...
%     
%     loopNearestIDs(i,:) = app.SavedData.BestStateMap(nearestPeakTimes);
%     loopWeights(i,:) = exp(-totalValues(nearestPeakTimes).^2/(2*sigma^2));
%     loopWeights(i,:) = loopWeights(i,:) ./ sum(loopWeights(i,:));
%     
% %     [~, bestClusters] = min(clusterDistances, [], 1);
% % 
% %     loopIDs = app.SavedData.BestStateMap(bestClusters,1);
%     
%     waitHandle.iterate(1);
% end
% close(waitHandle);
